...
interface PaymentCardProps {
  ...
  amountKobo: number;
  onPay: () => void;
  monthlyPayments?: Record<string, boolean>; // e.g. { '2026-06': true, '2026-07': false }
}
...
export function PaymentCard({
  ...
  const currentMonthNum = String(new Date().getMonth() + 1).padStart(2, '0'); // '01'-'12'
  ...
  return (
    <div className="shuttle-panel p-6 flex flex-col justify-between h-full bg-sl-panel">
      <div>
        ...
        {type === 'monthly' && (
          <div className="mb-6">
            <h4 className="text-xs font-bold uppercase text-sl-foreground mb-3 flex items-center gap-1">
              <span>📅</span> Membership Calendar ({currentYear})
            </h4>

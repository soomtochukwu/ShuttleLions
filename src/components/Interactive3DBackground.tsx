...
function createRacketMesh(colorGlow: string, pivotAtHandle = false): THREE.Group {
  const racketGroup = new THREE.Group();
  const yOffset = pivotAtHandle ? 5.4375 : 0;
  ...
  torusMesh.position.set(0, yOffset, 0);
  ...
  for (let i = -7; i <= 7; i++) {
    ...
    if (w > 0.1) {
      ...
      mesh.position.set(0, y + yOffset, 0);
      racketGroup.add(mesh);
    }
  }
  ...
  shaftMesh.position.set(0, -3.4375 + yOffset, 0); // Position below the hoop (-1.9375 - 1.5)
  ...
  jointMesh.position.set(0, -1.95 + yOffset, 0);
  ...
  handleMesh.position.set(0, -5.4375 + yOffset, 0); // Position below the shaft
  ...
  for (let i = 0; i < 4; i++) {
    ...
    ring.position.set(0, -5.0 - i * 0.24 + yOffset, 0);
  ...
  capMesh.position.set(0, -5.9675 + yOffset, 0);
...
export function Interactive3DBackground() {
  ...
  useEffect(() => {
    ...
    // Position coordinate references
    ...
    const getLandmarks = () => {
      return [
        { id: 0, x: serverX, y: 0.1, rotY: 0.0 },     // Section 1: Server point
        ...
        { id: 4, x: serverX, y: -32.1, rotY: 0.0 },   // Section 5: Receiver point
      ];
    };
    ...
    // Create 5 Standalone Rackets for Sections 1, 2, 3, 4, and 5
    const rackets: THREE.Group[] = [];
    const colors = ['#00c853', '#689f38', '#689f38', '#689f38', '#00c853'];
    for (let i = 0; i < 5; i++) {
      const racket = createRacketMesh(colors[i], false);
    ...
    const handleResize = () => {
      ...
      for (let i = 0; i < 5; i++) {
        rackets[i].position.set(landmarks[i].x, landmarks[i].y, -1.0);
      }
    };
    ...
    const animate = () => {
      ...
      if (isIntroActive) {
        // 1. INTRO LANDING PHASE (Racket 1 scales in, shuttlecock drops and bounces on it)
        introTime += delta;

        // Racket 1 scales in
        const r1Scale = Math.min(1.0, introTime * 1.5);
        rackets[0].scale.set(r1Scale, r1Scale, r1Scale);
        rackets[0].position.x = serverX;
        rackets[0].rotation.y = landmarks[0].rotY;

        // Realistic drop and bounce physics for the shuttlecock
        const dropDuration = 1.2;
        if (introTime < dropDuration) {
          const t = introTime / dropDuration;
          const height = 4.0 * (1.0 - t) * (1.0 - t);
          shuttle.position.set(serverX, landmarks[0].y + 0.15 + height, 0.15);
          shuttle.rotation.set(-Math.PI / 3 * (1.0 - t) + 0.4, 0, Math.PI / 4);
        } else {
          const bounceTime = introTime - dropDuration;
          const bounceFreq = 12.0;
          const decay = Math.exp(-bounceTime * 2.5);
          const bounceHeight = Math.abs(Math.sin(bounceTime * bounceFreq)) * 0.4 * decay;
          shuttle.position.set(serverX, landmarks[0].y + 0.15 + bounceHeight, 0.15);
      ...
      // 3. TRANSITION SECTIONS RACKETS (Rackets 1 to 5)
      rackets.forEach((racket, index) => {
        // Skip racket 1 during intro scale-in
        if (isIntroActive && index === 0) return;

        const racketY = landmarks[index].y;
        const dist = Math.abs(camera.position.y - racketY);

        let targetScale = 0;
        let targetRotY = landmarks[index].rotY;
        let targetX = landmarks[index].x;

        if (dist < 4.0 && !isIntroActive) {
          const factor = 1.0 - dist / 4.0;
          targetScale = Math.sin(factor * Math.PI / 2);
          targetRotY += (1 - factor) * (index % 2 === 0 ? 0.7 : -0.7);
        } else {
          targetScale = 0;
        }
        ...
        const slideX = targetScale > 0.01 ? targetX : (index % 2 === 0 ? 4.5 : -4.5);
